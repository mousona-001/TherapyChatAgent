	...
	useEffect(() => {
		async function findSessionInfo() {
			...
			try {
				...
				const currentSession = sessionResult.data.find(
				...
				if (currentSession?.connectionId) {
					...
					if (connResult.success && connResult.data) {
						const conn = connResult.data.find(
							(c: any) => c.id === currentSession.connectionId,
						);
						if (conn) {
							...
							setOtherPartyId(
								isPatient ? conn.therapistUserId : conn.patientUserId,
							);
							setInitialStatus(
								isPatient
									? conn.therapistStatus
									: (conn as any).patientStatus || "online",
							);
						}
					}
				} else {
