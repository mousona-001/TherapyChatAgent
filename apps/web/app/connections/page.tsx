...
export default function ConnectionsPage() {
	...
	const handleRespond = async (id: string, status: "accepted" | "rejected") => {
		...
		if (res.success) {
			toast.success(
				`Request ${status === "accepted" ? "approved" : "declined"} successfully`,
			);
			fetchData();
		} else {
				...
				<section>
					<div className="flex items-center justify-between mb-8">
						<div className="flex items-center gap-4">
							...
							<div>
								<h2 className="text-2xl font-bold text-[var(--on-surface)] tracking-tight">
									Pending Approval
								</h2>
								<p className="text-sm text-[var(--on-surface-variant)] opacity-60 font-medium">
									New patients waiting for your care
								</p>
							</div>
						</div>
						{pending.length > 0 && (
					...
					{loading ? (
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{[1, 2, 3].map((i) => (
								<Card
									key={i}
									className="p-6 h-[200px] animate-pulse bg-white border-[var(--outline-variant)]"
								/>
							))}
						</div>
					) : pending.length === 0 ? (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{pending.map((req) => (
								<Card
									key={req.id}
									className="p-6 flex flex-col gap-5 hover:shadow-xl hover:shadow-indigo-500/5 transition-all bg-white border-[var(--outline-variant)] relative overflow-hidden group"
								>
									...
									<div className="flex items-center gap-4 relative z-10">
										...
										<div className="flex-1 min-w-0">
											<h3 className="font-bold text-lg truncate group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
												{req.patientName}
											</h3>
					...
					<div className="flex items-center justify-between mb-8 pt-12 border-t border-[var(--outline-variant)]/50">
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-sm text-[var(--on-surface-variant)] opacity-60 font-medium">
									Your current caseload and wellness network
								</p>
							</div>
						</div>
						<div className="flex items-baseline gap-1">
							...
							<span className="text-[0.65rem] font-bold text-[var(--on-surface-variant)] uppercase opacity-40">
								Total
							</span>
						</div>
					</div>
					...
					{loading ? (
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
							{[1, 2, 3, 4].map((i) => (
								<Card
									key={i}
									className="p-6 h-[160px] animate-pulse bg-white border-[var(--outline-variant)]"
								/>
							))}
						</div>
					) : accepted.length === 0 ? (
						<Card className="p-12 text-center bg-[var(--surface-container-lowest)] border-none shadow-inner">
							<p className="text-sm font-medium text-[var(--on-surface-variant)] opacity-40 italic">
								Your active network will appear here after requests are
								accepted.
							</p>
						</Card>
					) : (
								...
								>
									<Card className="p-6 flex flex-col items-center text-center gap-4 hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer bg-white border-[var(--outline-variant)] group relative overflow-hidden">
										...
										<div className="relative z-10">
											<h3 className="font-bold text-base truncate max-w-[140px] uppercase tracking-tight group-hover:text-indigo-600 transition-colors">
												{patient.patientName}
											</h3>
											<div className="flex items-center justify-center gap-1.5 mt-1">
												<div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
												<p className="text-[0.65rem] text-[var(--on-surface-variant)] uppercase tracking-widest font-black opacity-30">
													Active Patient
												</p>
											</div>
										</div>
