...
function ChatPanel({
  ...
  const handleSend = async () => {
    ...
    const optimistic: Message = { content: text, role: "patient", senderId: "me", createdAt: new Date().toISOString() };
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[0.7rem] font-medium mt-0.5 flex items-center gap-1">
              ...
              />
              <span
                className={isOnline ? "text-emerald-700" : "text-on-surface-variant opacity-60"}
              >
                {isOnline ? "Online" : "Offline"}
              </span>
              {connection.therapistType && (
                <>
                  <span className="text-on-surface-variant opacity-30 mx-0.5">·</span>
...
export default function SessionsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const sessions: ChatSession[] = sessionResult.success ? (sessionResult.data ?? []) : [];
  ...
  const handleSelectThread = async (thread: SessionThread, conn: Connection) => {
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden">
      ...
      {!loading && !error && groups.length === 0 && requestedConnections.length === 0 ? (
        <NoConnectionsEmpty />
      ) : (
        ...
        <div className="flex-1 overflow-y-auto py-2 min-h-0">
          {loading ? (
            ...
            <>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            {creating ? (
                              ...
                              group.isExpanded ? (
                                <CaretDown className="w-3 h-3 text-on-surface-variant opacity-50" weight="bold" />
                              ) : (
                                <CaretRight className="w-3 h-3 text-on-surface-variant opacity-50" weight="bold" />
                              )
                            ) : (
                            ...
                            {group.threads.map((thread) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => handleSelectThread(thread, conn)}
                  ...
                  {showRequested && requestedConnections.map((conn) => {
                    const initials = getInitials(conn.therapistName);
                    return (
      ...
      <div className={`flex-1 flex flex-col h-full min-w-0 ${selectedSessionId ? "flex" : "hidden md:flex"}`}>
