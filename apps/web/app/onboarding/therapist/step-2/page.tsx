...
import { z } from "zod";
import { User, DeviceMobile, Translate, CaretDown } from "@phosphor-icons/react";
...
import { saveTherapistProfile, getProfile, resetPhoneVerification } from "../../actions";
...
export default function TherapistStep2() {
  ...
  const [dateOfBirth, setDateOfBirth] = useLocalStorage<Date>("therapist_dateOfBirth", new Date());
  const [gender, setGender] = useLocalStorage("therapist_gender", "");
  const [bio, setBio] = useLocalStorage("therapist_bio", "");
  const [phoneNumber, setPhoneNumber] = useLocalStorage("therapist_phoneNumber", "");
  const [languages, setLanguages] = useLocalStorage<string[]>("therapist_languages", ["English"]);
  const [isVerified, setIsVerified] = useLocalStorage("therapist_isVerified", false);
  ...
  return (
    <OnboardingLayout>
      ...
      <p className="text-[0.95rem] font-normal text-[var(--on-surface-variant)] text-center leading-[1.65] max-w-[440px] mb-[3rem]">
        This forms your provider profile. Patients and your care team will use this to understand
        who you are and how best to work with you.
      </p>
      ...
      <FormCard>
        ...
        {errors.phoneNumber && <p className="text-red-500 text-[0.7rem] mt-2">{errors.phoneNumber}</p>}
      </FormCard>
      ...
      <FormCard>
        <FormGroup>
          ...
          {errors.languages && <p className="text-red-500 text-[0.7rem] mt-1">{errors.languages}</p>}
        </FormGroup>
      </FormCard>
