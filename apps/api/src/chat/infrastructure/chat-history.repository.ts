...
import { ChatMessageMongo, ChatMessageDocument } from './schemas/chat-message.schema';
import { eq, asc, desc, and, or, inArray } from 'drizzle-orm';
...
export class ChatHistoryRepository {
  ...
  async getOrCreateSession(userId: string): Promise<{
    ...
    return {
      sessionId,
      messages: docs.map(d => ({ role: d.role, content: d.content, senderId: d.senderId })),
    };
  }
  ...
  async addMessage(sessionId: string, role: 'user' | 'assistant', content: string, senderId?: string): Promise<void> {
  ...
  async getSessions(userId: string) {
    ...
    const userConnections = await db
      ...
      .leftJoin(patientProfile, eq(therapistConnection.patientId, patientProfile.id))
      .leftJoin(therapistProfile, eq(therapistConnection.therapistId, therapistProfile.id))
      .where(or(
  ...
  async getMessages(sessionId: string): Promise<(ChatMessageRecord & { createdAt: Date })[]> {
    const docs = await this.messageModel
      .find({ sessionId })
      .sort({ createdAt: 1 })
      .lean()
  ...
  async updateSessionMemory(
    sessionId: string,
    data: { chatSummary?: string; emotionalState?: string; lastTopics?: string },
  ): Promise<void> {
    await db
      .update(chatSession)
