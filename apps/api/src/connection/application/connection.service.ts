...
export class ConnectionService {
	...
	async requestConnection(patientUserId: string, dto: RequestConnectionDto) {
		...
		if (existing.length > 0 && existing[0].status !== "rejected") {
			throw new BadRequestException(
				`Connection already exists with status: ${existing[0].status}`,
			);
		}
		...
		const [connection] = await db
			...
			.onConflictDoUpdate({
				target: [
					therapistConnection.patientId,
					therapistConnection.therapistId,
				],
	...
	async respondToConnection(
		...
		if (connection.status !== "pending") {
			throw new BadRequestException(
				`Cannot respond to a connection with status: ${connection.status}`,
			);
		}
	...
	async submitFeedback(
		...
		if (!connection)
			throw new NotFoundException("Connection not found or permission denied");
		if (connection.status !== "accepted" && connection.status !== "ended") {
			throw new BadRequestException(
				"Can only give feedback for active or ended connections",
			);
		}
	...
	private async updateTherapistAverageRating(therapistId: string) {
		...
		const validRatings = connections
